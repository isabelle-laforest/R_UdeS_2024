...
```{r}
ggplot(familyComp.df, aes(x = mean_count, y = Family, fill = Compartment)) + 
  geom_col(position = position_dodge(width = 1)) + # slight modificatino
  geom_errorbar(aes(xmin = mean_count - sd_count, 
                    xmax = mean_count + sd_count),
                width = 0.3, # size of the "caps"
                size = 0.2, # thickness
                position = position_dodge(width = 1)) # same as geom_col
...
myplot <- # save this plot as an object
  ...
  geom_errorbar(aes(xmin = mean_count - sd_count, xmax = mean_count + sd_count),
                width = 0.3, size = 0.2, position = position_dodge(width = 0.7)) +
...
ggplot(seq_count, aes(y = Host, x = seq_count)) + 
  geom_bar(aes(fill = Family),  # Using geom_bar instead of geom_col !
...
```{r}
ggplot(seq_count, aes(y = Host, x = mean_count)) + 
