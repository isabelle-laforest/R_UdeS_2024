...
We want to look at how microbial families are distributed across our samples using their sequence counts. We'll start very simple, by using the sum of all samples for each Family.

```{r}
ggplot(seq_count, aes(x = mean_count, y = Family)) + 
  geom_col() 
```
...
```{r}
ggplot(seq_count, aes(x = Seq_count, y = Family)) + 
...
ggplot(seq_count, aes(x = Seq_count, y = Family)) + 
  ...
  labs(title = "Total Family sequence counts by compartment.",
...
A different way of looking at the data could be to compare Host and Compartment at the same time. To visualize the families across these two sample data, we can use faceting.
