...
melt <- psMoss %>% psmelt %>% 
  select(OTU, Sample, Abundance, Compartment, Microsite, Host, Order)

topTaxa <- melt %>%  
  group_by(Order) %>% # melt the table and group by tax level
  ...
  mutate(aggTaxo = as.factor(case_when( # aggTaxo will become the plot legend
    row_number()<=6 ~ Order, #+++ We'll need to manually order the species!
    row_number()>6~'Others'))) %>%  # +1 to include the Others section!
  select(-Abundance)

subset <- left_join(melt, topTaxa, by = 'Order') %>% 
  aggregate(Abundance~Sample+aggTaxo+Host+Compartment, # Abundance is aggregated...
            data=., FUN = sum) %>% 
  dplyr::rename(Order = aggTaxo) %>% 
